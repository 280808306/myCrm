		...
		var cmdObject = {
			...
			editBean : function() {
				...
				if (row) {
					...
					$.get("/contractItem/listByParent",{id:row.id},function(data){
			...
			saveBean : function() {
				beanForm.form('submit', {
					...
					onSubmit : function(param) {
						var rows = listItemDataGrid.datagrid("getRows");
						console.debug(rows);
						for(var i=0;i<rows.length;i++){
							for(var name in rows[i]){
								if(rows[i][name]){
									param["details["+i+"]."+name] = rows[i][name];
								}
							}
						}
						console.debug(param);
...
<body>
	...
	<div id="editBeanDialog" class="easyui-dialog" style="width: 900px; height: 400px; padding: 10px 20px" closed="true" buttons="#editDialogButtons">
		<div class="easyui-layout" data-options="fit:true">
			<div data-options="region:'west',split:true" style="width: 420px; height: 300px; padding: 10px;">
				<div class="easyui-panel" data-options="fit:true,title:'合同信息'">
			...
			<div data-options="region:'east',split:true" style="width: 420px; height: 300px; padding: 10px;">
				<div class="easyui-panel" data-options="fit:true,title:'合同详细'">
	...
	<div id="editItemDialog" class="easyui-dialog" style="width: 400px; height: 280px; padding: 10px 20px" closed="true" buttons="#editItemButtons">
		<form id="editItemForm" class="dialog-form" method="post" novalidate>
			...
			<div class="fitem">
				<label>支付时间:</label>
				<input name="payTimeStr" class="easyui-datebox">
			</div>
			<div class="fitem">
				<label>支付金额:</label>
				<input name="money" class="easyui-validatebox"></input>
			</div>
			<div class="fitem">
				<label>是否已付:</label>
				<input type="radio" name="isPayment" value="1">是</input>
				<input type="radio" name="isPayment" checked="checked" value="0">否</input>
			</div>
		</form>
	</div>
